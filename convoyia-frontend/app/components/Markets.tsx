...
const phases = [
  {
    phase: "M0 — Live",
    markets: ["🇫🇷 France (Goweyy)"],
    status: "live",
  },
  {
    phase: "M6 — Q1 2027",
    markets: ["🇬🇧 United Kingdom", "🇧🇪 Belgium", "🇳🇱 Netherlands"],
    status: "soon",
  },
  {
    phase: "M12 — Q3 2027",
    markets: ["🇩🇪 Germany", "🇪🇸 Spain", "🇮🇹 Italy"],
    status: "planned",
  },
  {
    phase: "M18 — Q1 2028",
  ...
  {
    phase: "M24 — Q3 2028",
    markets: ["🇦🇺 Australia", "🇨🇦 Canada", "🇺🇸 United States"],
    status: "planned",
  },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl mx-auto text-center mb-16">
          ...
          <p className="text-white/55 text-lg">
            Every dimension that varies per country is a tenant configuration.
            Launch in a new market by updating a JSON config, not rewriting code.
          </p>
        </div>
          ...
          {phases.map((p) => (
            <div key={p.phase} className="card-glass p-5 hover:border-white/20 transition-colors">
              <div className="flex items-center justify-between mb-3">
                ...
                <span
                  className={`text-[11px] font-bold px-2 py-0.5 rounded-full border ${statusStyle[p.status]}`}
                >
        ...
        {/* Config dimensions table */}
        <h3 className="font-bold text-white/60 text-sm uppercase tracking-widest mb-5">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10">
                <th className="text-left px-5 py-3 text-white/40 font-semibold text-xs uppercase tracking-wider">
                  Dimension
                </th>
                <th className="text-left px-5 py-3 text-white/40 font-semibold text-xs uppercase tracking-wider">
                  Goweyy (France)
