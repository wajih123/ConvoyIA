...
import { Navigation2, ShieldCheck, TrendingUp, Camera, MapPin, Receipt } from "lucide-react";
    ...
    <section id="agents" className="py-24 border-t border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl mx-auto text-center mb-16">
          <p className="section-label"><Navigation2 size={14} /> AI Agents</p>
          ...
          <p className="text-white/55 text-lg">
            ...
            Kafka topics, and LLM model. Use all six or compose only what you need.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {agents.map((a) => (
            <div
              key={a.id}
              className={`card-glass p-6 flex flex-col cursor-pointer transition-all duration-200 ${
                active === a.id ? "border-white/30 shadow-lg shadow-black/20" : "hover:border-white/20"
              }`}
              onClick={() => setActive(active === a.id ? null : a.id)}
            >
              <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${a.color} flex items-center justify-center text-white mb-5 shadow-lg`}>
                {a.icon}
              </div>
              ...
              {/* Expandable features */}
              <div className={`overflow-hidden transition-all duration-300 ${active === a.id ? "max-h-40" : "max-h-0"}`}>
                <ul className="space-y-1.5 mb-4">
                  {a.features.map((f) => (
                    <li key={f} className="flex items-center gap-2 text-xs text-white/55">
                      <span className="w-1.5 h-1.5 rounded-full bg-brand-400 shrink-0" />{f}
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div className="flex items-center justify-between mt-auto pt-3 border-t border-white/8">
                <span className="text-[11px] text-white/35 font-mono">{a.model}</span>
                <span className="text-[11px] text-brand-400 font-medium">
                  {active === a.id ? "Hide details ↑" : "Show details ↓"}
                </span>
              </div>
            </div>
          ))}
