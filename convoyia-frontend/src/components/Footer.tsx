    ...
    <footer className="border-t border-white/5 pt-14 pb-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-8 mb-12">
          ...
          {cols.map((c) => (
            <div key={c.title}>
              <div className="text-xs font-bold uppercase tracking-widest text-white/30 mb-4">{c.title}</div>
        ...
        <div className="border-t border-white/5 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-white/30">© {new Date().getFullYear()} ConvoyIA. All rights reserved.</p>
          <p className="text-xs text-white/25">
            Powering <span className="text-white/45">Goweyy</span> — Nice, France · Built on Hetzner + K8s
          </p>
        </div>
      </div>
    </footer>
